...
from api.monitoring.metrics import get_metrics_text, get_metrics_content_type
...
async def prometheus_metrics(request: Request):
    ...
    try:
        db_pool = request.app.state.db_pool
        if db_pool:
            from api.monitoring.metrics import update_db_connections
    ...
    try:
        redis_client = getattr(request.app.state, 'redis_client', None)
        if redis_client:
            from api.monitoring.metrics import update_cache_hit_rate
